...
def update_ios_version(project_root, bundle_short_version, bundle_version):
    """更新iOS平台版本信息
...
def main():
    """主函数，用于独立运行此脚本"""
    if len(sys.argv) < 4:
        print("用法: python update_ios_version.py <项目根目录> <版本名称> <构建号>")
        print("示例: python update_ios_version.py . 1.0.0 20250620001")
        sys.exit(1)
    ...
    if update_ios_version(project_root, bundle_short_version, bundle_version):
        print("iOS版本信息更新成功")
    else:
        print("iOS版本信息更新失败")
        sys.exit(1)
