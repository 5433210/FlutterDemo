...
mixin CanvasControlPointHandlers {
  ...
  Map<String, double> calculateFinalElementProperties(
    ...
    if (controller.state.selectedElementIds.length == 1) {
      ...
      final alignedProperties =
          _applyGuideLineAlignment(elementId, finalProperties);
      finalProperties.addAll(alignedProperties);
    }
  ...
  void handleControlPointLiveUpdate(Map<String, double> liveState) {
    ...
    // 🔧 在Live阶段应用网格吸附
    var processedLiveState = controller.state.snapEnabled
        ? _applyGridSnapToProperties(liveState)
        : liveState;

    // 🔧 新增：在Live阶段应用参考线对齐（只在单选状态下）
    processedLiveState =
        _applyGuideLineAlignment(elementId, processedLiveState);

    // 🚀 新增：对组合元素进行Live阶段的子元素预览更新
    if (originalElement['type'] == 'group') {
      _handleGroupElementLiveUpdate(originalElement, processedLiveState);
    } else {
      // 普通元素的Live更新
      _handleSingleElementLiveUpdate(
          elementId, originalElement, processedLiveState);
    }
  }
  ...
  void handleControlPointUpdate(int controlPointIndex, Offset delta) {
    ...
    // 🎯 特殊处理：控制点索引-3表示来自FreeControlPoints的参考线对齐检测请求
    if (controlPointIndex == -3) {
      EditPageLogger.canvasDebug('接收到FreeControlPoints参考线对齐检测请求', data: {
        'elementId': controller.state.selectedElementIds.isNotEmpty ? controller.state.selectedElementIds.first : 'none',
        'alignmentMode': AlignmentModeManager.currentMode.toString(),
        'operation': 'alignment_detection_request_received',
      });
      ...
      if (AlignmentModeManager.currentMode == AlignmentMode.guideLine &&
        ...
        // 通过SmartCanvasGestureHandler的对齐处理器执行对齐检测
        // 获取当前元素状态并转换为拖拽偏移量进行对齐检测
        final currentPageElements = controller.state.currentPageElements;
        final currentElement = currentPageElements.firstWhere(
          (e) => e['id'] == elementId,
          orElse: () => <String, dynamic>{},
        );
        ...
        if (currentElement.isNotEmpty) {
          // 获取当前元素的最新位置
          final currentX = (currentElement['x'] as num?)?.toDouble() ?? 0.0;
          final currentY = (currentElement['y'] as num?)?.toDouble() ?? 0.0;
          final currentWidth = (currentElement['width'] as num?)?.toDouble() ?? 0.0;
          final currentHeight = (currentElement['height'] as num?)?.toDouble() ?? 0.0;
          
          EditPageLogger.canvasDebug('FreeControlPoints对齐检测 - 元素当前状态', data: {
            'elementId': elementId,
            'position': '($currentX, $currentY)',
            'size': '($currentWidth x $currentHeight)',
            'operation': 'free_control_points_element_state_check',
          });

          // 🔧 关键：通过SmartCanvasGestureHandler的公共方法触发对齐检测
          gestureHandler.triggerAlignmentDetectionForElement(elementId);
          
          EditPageLogger.canvasDebug('FreeControlPoints已触发SmartCanvasGestureHandler对齐检测', data: {
            'elementId': elementId,
            'method': 'triggerAlignmentDetectionForElement',
            'operation': 'free_control_points_alignment_detection_executed',
          });
        }
      }
    ...
    // 🎯 关键新增：在控制点更新时触发参考线对齐检测
    if (AlignmentModeManager.currentMode == AlignmentMode.guideLine &&
        controller.state.selectedElementIds.isNotEmpty) {
      final elementId = controller.state.selectedElementIds.first;

      EditPageLogger.canvasDebug('控制点触发参考线对齐检测', data: {
        'elementId': elementId,
        'controlPointIndex': controlPointIndex,
        'alignmentMode': AlignmentModeManager.currentMode.toString(),
        'operation': 'control_point_alignment_trigger',
      });

      // TODO: 在这里实现参考线对齐检测逻辑
      // 类似SmartCanvasGestureHandler中的对齐检测
    }
  ...
  Map<String, double> _applyGridSnapToProperties(
    ...
    final result = Map<String, double>.from(properties);

    // 只有在网格对齐模式下才应用网格吸附
    if (AlignmentModeManager.currentMode == AlignmentMode.grid) {
      if (result.containsKey('x')) {
        result['x'] = _snapToGrid(result['x']!);
      }
      if (result.containsKey('y')) {
        result['y'] = _snapToGrid(result['y']!);
      }
    }
  ...
  /// 应用参考线对齐
  Map<String, double> _applyGuideLineAlignment(
      String elementId, Map<String, double> properties) {
    final result = Map<String, double>.from(properties);
    ...
    EditPageLogger.canvasDebug('开始参考线对齐检测', data: {
      'elementId': elementId,
      'x': result['x'],
      'y': result['y'],
      'width': result['width'],
    ...
    try {
      ...
      tempElement['x'] = result['x'] ?? element['x'];
      tempElement['y'] = result['y'] ?? element['y'];
      tempElement['width'] = result['width'] ?? element['width'];
      tempElement['height'] = result['height'] ?? element['height'];

      // 获取其他元素用于对齐检测
      final otherElements = controller.state.currentPageElements
          .where((e) => e['id'] != elementId)
          .toList();

      // 进行对齐检测
      final alignments =
          AlignmentDetector.detectAlignments(tempElement, otherElements);

      EditPageLogger.canvasDebug('参考线对齐检测结果', data: {
        'elementId': elementId,
      ...
      if (alignments.isNotEmpty) {
        for (final alignment in alignments) {
          if (alignment.sourceLine.orientation ==
              GuideLineOrientation.horizontal) {
            result['y'] = (result['y'] ?? tempElement['y'] as double) +
                alignment.adjustment.dy;
          } else {
            result['x'] = (result['x'] ?? tempElement['x'] as double) +
                alignment.adjustment.dx;
          }
        }

        EditPageLogger.canvasDebug('应用参考线对齐调整', data: {
          'elementId': elementId,
          'adjustedX': result['x'],
          'adjustedY': result['y'],
      ...
      EditPageLogger.canvasError('参考线对齐检测异常', error: e, stackTrace: stackTrace);
    }
