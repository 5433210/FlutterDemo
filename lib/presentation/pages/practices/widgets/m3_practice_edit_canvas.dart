...
import 'free_control_points.dart'; // 导入带旋转功能的测试控制点
...
class _M3PracticeEditCanvasState extends State<M3PracticeEditCanvas> {
  ...
  final bool _shouldIgnoreGestureDetector = false;
  ...
  Widget _buildInteractionLayer(LayerConfig config) {
    ...
    return Stack(
      ...
      children: [
        ...
        // Control points - switch between test mode and normal mode
        if (selectedElementId != null)
          Positioned.fill(
            child: _isTestMode
                ? _buildTestControlPoints(
                    selectedElementId, x, y, width, height, rotation)
                : _buildControlPoints(
                    selectedElementId, x, y, width, height, rotation),
          ),
        // Test mode toggle button (top-right corner)
        Positioned(
          top: 20,
          right: 20,
          child: Material(
            ...
            child: InkWell(
              onTap: () {
                ...
                debugPrint('🧪 测试模式切换: ${_isTestMode ? "开启" : "关闭"}');
              },
              borderRadius: BorderRadius.circular(8),
              child: Padding(
                padding:
                ...
                child: Row(
                  ...
                  children: [
                    Icon(
                      ...
                      color: _isTestMode ? Colors.orange : Colors.white,
                      size: 16,
                    ),
                    const SizedBox(width: 4),
                    Text(
                      _isTestMode ? '测试模式' : '正常模式',
                      style: TextStyle(
                        color: _isTestMode ? Colors.orange : Colors.white,
                        fontSize: 12,
  ...
  /// Build test control points - independent movement not bound to element
  Widget _buildTestControlPoints(
    String elementId,
    double x,
    double y,
    double width,
    ...
    debugPrint(
        '🧪 构建测试控制点 - 元素ID: $elementId, 坐标: ($x, $y), 尺寸: ${width}x$height');
        ...
        child: Stack(
          children: [
            ...
            Positioned(
              ...
              bottom: 0,
              child: RepaintBoundary(
                key: ValueKey('test_control_points_$elementId'),
                child: FreeControlPoints(
                  key: ValueKey(
                      'test_control_points_${elementId}_${scale.toStringAsFixed(2)}'),
                  elementId: elementId,
                  x: x,
                  y: y,
                  width: width,
  ...
  void _handleControlPointUpdate(int controlPointIndex, Offset delta) {
    // 注意：控制点的delta已经是在正确的坐标系中计算的，
    // 不需要额外的缩放调整，直接使用原始delta确保鼠标跟踪的准确性
    //
    // 之前的实现：final adjustedDelta = Offset(delta.dx / scale, delta.dy / scale);
    // 这会导致在高缩放比例下控制点移动滞后于鼠标移动，因为除以scale会缩小移动距离
    //
    // 正确的做法是直接使用原始delta，因为：
    // 1. canvas_control_points.dart 已经在正确的坐标系中计算了delta
    // 2. InteractiveViewer的变换矩阵会自动处理坐标系转换
    // 3. 控制点应该直接跟随鼠标移动，而不需要额外的缩放补偿
