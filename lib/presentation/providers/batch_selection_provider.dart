...
class BatchSelectionState with _$BatchSelectionState {
  ...
  bool get hasSelection =>
      selectedWorkIds.isNotEmpty || selectedCharacterIds.isNotEmpty;
...
class BatchSelectionNotifier extends StateNotifier<BatchSelectionState> {
  ...
  void updateAllSelectedState(int totalItems) {
    ...
    final newIsAllSelected =
        currentSelected > 0 && currentSelected == totalItems;
...
final worksBatchSelectionProvider = Provider<BatchSelectionState>((ref) {
  ...
  return state.pageType == PageType.works
      ? state
      : state.copyWith(pageType: PageType.works);
});

/// 集字页面批量选择Provider（便捷访问）
final charactersBatchSelectionProvider = Provider<BatchSelectionState>((ref) {
  final state = ref.watch(batchSelectionProvider);
  return state.pageType == PageType.characters
      ? state
      : state.copyWith(pageType: PageType.characters);
});

/// 批量操作可用性Provider
final batchOperationsAvailableProvider =
    Provider<Map<BatchOperation, bool>>((ref) {
  final state = ref.watch(batchSelectionProvider);
