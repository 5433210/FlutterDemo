...
class _WorkImportPreviewState extends State<WorkImportPreview> {
    ...
    return Column(
      ...
      children: [
        PreviewToolbar(
          ...
          onRotateLeft:
              hasSelection ? () => widget.viewModel.rotateImage(false) : null,
          onRotateRight:
              hasSelection ? () => widget.viewModel.rotateImage(true) : null,
          onDelete: hasSelection
              ? () =>
                  widget.viewModel.removeImage(widget.state.selectedImageIndex)
              : null,
  ...
  Future<void> _handleDropDone(DropDoneDetails details) async {
    ...
    try {
      // 获取当前已加载的文件路径
      final existingPaths =
          Set<String>.from(widget.state.images.map((file) => file.path));
