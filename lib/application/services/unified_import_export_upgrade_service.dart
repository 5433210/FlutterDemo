...
class UnifiedImportExportUpgradeService {
  ...
  Future<void> initialize() async {
    ...
    try {
      ...
      _isInitialized = true;
      AppLogger.info('统一导入导出升级服务初始化完成',
  ...
  Future<String?> detectExportDataVersion(String exportFilePath) async {
    try {
      ...
      // 根据文件扩展名和内容检测版本
      final extension = exportFilePath.toLowerCase();
    ...
    try {
      ...
      if (exportDataVersion == null) {
        AppLogger.warning('无法检测导出文件版本',
            data: {'exportFile': exportFilePath},
            tag: 'UnifiedImportExportUpgradeService');
      ...
      switch (compatibility) {
        ...
        case ImportExportCompatibility.upgradable:
          return await _performUpgrade(
              sourceVersion, targetVersion, exportFilePath);

        case ImportExportCompatibility.appUpgradeRequired:
          return ImportUpgradeResult.appUpgradeRequired(
  ...
  String getUpgradeSuggestion(String exportFilePath, String currentAppVersion) {
    try {
      ...
      if (extension.endsWith('.json')) {
        // 可能是 ie_v1 格式
        return ImportExportVersionMappingService.getUpgradeSuggestion(
            'ie_v1',
            ImportExportVersionMappingService.getDataVersionForApp(
                currentAppVersion));
      } else if (extension.endsWith('.zip') ||
  ...
  String getCompatibilityDescription(ImportExportCompatibility compatibility) {
    return ImportExportVersionMappingService.getCompatibilityDescription(
        compatibility);
  }
  ...
  Future<ImportUpgradeResult> _performUpgrade(
    ...
    try {
      ...
      // 1. 获取升级路径
      final adapters =
          _adapterManager.getUpgradePath(sourceVersion, targetVersion);
      if (adapters.isEmpty) {
        return ImportUpgradeResult.error(
            '无法找到从 $sourceVersion 到 $targetVersion 的升级路径');
      }

      // 2. 执行升级链
      final upgradeChainResult =
          await _adapterManager.executeUpgradeChain(adapters, exportFilePath);
      ...
      if (upgradeChainResult.finalOutputPath != null) {
        await _adapterManager.executePostProcessing(
            adapters, upgradeChainResult.finalOutputPath!);
      }
