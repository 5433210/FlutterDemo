...
void main() {
  group('MemoryManager Tests', () {
    ...
    setUp(() {
      memoryManager = MemoryManager(maxMemoryBytes: 50 * 1024 * 1024); // 50MB for testing
    });
    ...
    test('内存管理器应该正确初始化', () {
      ...
      print('📊 初始状态：');
      print('   最大内存限制: ${(stats.maxLimit / (1024 * 1024)).toStringAsFixed(1)}MB');
      print('   当前使用量: ${(stats.currentUsage / (1024 * 1024)).toStringAsFixed(1)}MB');
    ...
    test('内存效率元素创建应该正确工作', () {
      ...
      final efficientElement = memoryManager.createMemoryEfficientElement(testElement);
      
      print('📊 效率元素信息：');
      print('   元素ID: ${efficientElement.id}');
      print('   元素类型: ${efficientElement.type}');
      print('   估计大小: ${(efficientElement.estimatedSize / 1024).toStringAsFixed(1)}KB');
      print('   是否为大元素: ${efficientElement.isLarge}');
      print('   边界: ${efficientElement.bounds.x}, ${efficientElement.bounds.y}, ${efficientElement.bounds.width}x${efficientElement.bounds.height}');
    ...
    test('内存清理应该有效工作', () async {
      ...
      print('📊 清理前状态：');
      print('   使用量: ${(beforeStats.currentUsage / (1024 * 1024)).toStringAsFixed(1)}MB');
      print('   元素数: ${beforeStats.trackedElementCount}');
      
      // 执行内存清理
      final freedBytes = await memoryManager.performMemoryCleanup(aggressive: true);
      
      final afterStats = memoryManager.memoryStats;
      print('📊 清理后状态：');
      print('   使用量: ${(afterStats.currentUsage / (1024 * 1024)).toStringAsFixed(1)}MB');
      ...
      expect(afterStats.currentUsage, lessThanOrEqualTo(beforeStats.currentUsage));
    ...
    test('内存限制调整应该正确工作', () {
      ...
      // 调整内存限制
      final newLimit = 100 * 1024 * 1024; // 100MB
      memoryManager.adjustMemoryLimits(newMaxMemory: newLimit);
      
      final adjustedStats = memoryManager.memoryStats;
      print('📊 调整后限制: ${(adjustedStats.maxLimit / (1024 * 1024)).toStringAsFixed(1)}MB');
    ...
    test('元素卸载应该正确工作', () {
      ...
      // 卸载元素
      final unloaded = memoryManager.unregisterElementMemory('unload_element_1');
      ...
      expect(afterStats.trackedElementCount, equals(beforeStats.trackedElementCount - 1));
  ...
  group('MemoryStats Tests', () {
    test('内存统计信息应该准确计算', () {
      ...
      final memoryManager = MemoryManager(maxMemoryBytes: 20 * 1024 * 1024); // 20MB
      ...
      print('📊 内存统计信息：');
      print('   当前使用量: ${(stats.currentUsage / (1024 * 1024)).toStringAsFixed(1)}MB');
      print('   峰值使用量: ${(stats.peakUsage / (1024 * 1024)).toStringAsFixed(1)}MB');
      print('   最大限制: ${(stats.maxLimit / (1024 * 1024)).toStringAsFixed(1)}MB');
      print('   压力比例: ${(stats.pressureRatio * 100).toStringAsFixed(1)}%');
  ...
  group('ElementBounds Tests', () {
    test('元素边界计算应该正确工作', () {
      ...
      print('📊 边界测试：');
      print('   边界1: (${bounds1.x}, ${bounds1.y}) ${bounds1.width}x${bounds1.height}');
      print('   边界2: (${bounds2.x}, ${bounds2.y}) ${bounds2.width}x${bounds2.height}');
      print('   边界3: (${bounds3.x}, ${bounds3.y}) ${bounds3.width}x${bounds3.height}');
