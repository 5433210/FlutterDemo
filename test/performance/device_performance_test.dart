...
void main() {
  group('DevicePerformanceDetector Tests', () {
    test('设备性能检测应该返回有效的性能等级', () async {
      ...
      final performanceLevel = await DevicePerformanceDetector.detectDevicePerformance();
      stopwatch.stop();
    ...
    test('不同性能等级配置应该有明显区别', () {
      ...
      print('📊 配置对比：');
      print('   低性能: 帧率=${lowConfig.maxFrameRate}, 质量=${lowConfig.renderQuality}');
      print('   中等性能: 帧率=${mediumConfig.maxFrameRate}, 质量=${mediumConfig.renderQuality}');
      print('   高性能: 帧率=${highConfig.maxFrameRate}, 质量=${highConfig.renderQuality}');
      
      // 验证配置递进关系
      expect(lowConfig.renderQuality, lessThan(mediumConfig.renderQuality));
      expect(mediumConfig.renderQuality, lessThanOrEqualTo(highConfig.renderQuality));
      expect(lowConfig.maxFrameRate, lessThanOrEqualTo(mediumConfig.maxFrameRate));
    ...
    test('自适应性能优化器应该正确初始化', () async {
      ...
      // 先检测设备性能
      final detectedLevel = await DevicePerformanceDetector.detectDevicePerformance();
      print('📊 检测到设备性能: $detectedLevel');
      
      final optimizer = SelfAdaptivePerformanceOptimizer(
        memoryManager: memoryManager,
        performanceMonitor: performanceMonitor,
        detectedPerformanceLevel: detectedLevel,
      );
    ...
    test('自适应优化器应该能够正确调整配置', () async {
      ...
      expect(optimizer.devicePerformanceLevel, DevicePerformanceLevel.low); // 最后设置的值
  ...
  group('PerformanceOptimizationConfig Tests', () {
    ...
    test('高内存压力配置应该降低资源使用', () {
      ...
      print('📊 配置对比：');
      print('   正常: 质量=${normalConfig.renderQuality}, 缓存=${normalConfig.cacheLimit}');
      print('   内存压力: 质量=${memoryConfig.renderQuality}, 缓存=${memoryConfig.cacheLimit}');
      
      // 内存压力配置应该降低资源使用
      expect(memoryConfig.renderQuality, lessThanOrEqualTo(normalConfig.renderQuality));
      expect(memoryConfig.cacheLimit, lessThanOrEqualTo(normalConfig.cacheLimit));
  ...
  group('性能等级集成测试', () {
    test('完整的性能检测到配置应用流程', () async {
      ...
      print('📊 第3步 - 配置验证: 帧率=${config.maxFrameRate}, 质量=${config.renderQuality}');
      ...
      expect(report, isNotNull);
      expect(report['devicePerformanceLevel'], contains(detectedLevel.toString().split('.').last));
