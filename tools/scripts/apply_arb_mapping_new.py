    ...
    for line in lines:
        ...
        if not line.startswith(' '):
            if ': ' in line:
                ...
                # If we're in a replacement section or about to process replacements
                if is_replacement_section:
                    current_replacing_key = key
                    replacing_keys[key] = []
                    key_mapping[key] = key  # Map to itself
                    is_replacement_section = False  # Reset after processing one key
                else:
                    # It's a normal key
                    current_replacing_key = None
                    key_mapping[key] = key  # Map to itself
